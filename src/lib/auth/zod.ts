...
export const signupSchema = object({
  name: string({ required_error: 'Name is requierd' }).min(4).max(40).regex(/[A-Z]/, 'Password must contain at least one uppercase letter'),
  email: string({ required_error: 'Email is requierd' }).email({message: 'Enter email correctly.'}),
  username: string({ required_error: 'Username is requierd' }).min(1).max(20),
  password: string({ required_error: 'Password is requierd' }).min(2).max(20),
